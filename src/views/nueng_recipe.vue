<template>
  <br>
    ...
    <router-link to="/nueng_recipe"><li><a :class="{ 'actives': isActive('/nueng_recipe') }">นึ่ง</a></li></router-link>


    
  </ul>
  <div v-if="recipes.length > 0" class="flex items-center justify-center">
  ...
  const fetch_recipes = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API}/nueng`) 
